/* ==========================================================================
   OMWANDI TIMEKEEPER - REFACTORED MASTER CLIENT
   ========================================================================== */
...
function checkAuth() {
    ...
    if (state.activeProfileId) {
        ...
        if (me) {
            ...
            if (state.userRole === 'Employee' || state.userRole === 'Viewer' || isMobile()) state.activeView = 'timer';
...
function updateUIForUser(me) {
    ...
    if (state.userRole === 'Employee' || state.userRole === 'Viewer' || isMobile()) {
        ...
        if (isMobile() && state.userRole !== 'Employee') {
...
function renderViewHeader(title) {
    ...
    return `<div class="view-header"><h2>${title}</h2>${logoutBtn}</div>`;
}
...
function renderDashboard(container) {
    ...
    const grouped = {};
    active.forEach(t => { if (!grouped[t.project_id]) grouped[t.project_id] = []; grouped[t.project_id].push(t); });
    
    let listHtml = active.length === 0 ? '<div style="text-align:center; padding:40px; color:var(--text-muted);">No active sessions found.</div>' : 
...
function renderTimer(container) {
    const active = state.timeEntries.find(e => e.employee_id === state.activeProfileId && (e.total_hours === 0 || !e.end_time));
...
function renderProjects(container) {
    const list = state.projects.map(p => {
        ...
        return `<div class="glass-container project-card-v2">
            <div class="card-header"><h3>${p.name}</h3><span class="project-indicator" style="background:${p.color || '#6366f1'}; width:12px; height:12px; border-radius:3px;"></span></div>
    ...
    container.innerHTML = `
        ...
        <div class="glass-container" style="margin-bottom:24px; border-left: 4px solid #8b5cf6;">
            ...
            <div id="mappingForm" class="settings-form" style="margin-top:20px;">
                <div style="display:grid; grid-template-columns: 1fr 1fr; gap:16px; margin-bottom:16px;">
                    <div><label style="font-size:0.7rem; font-weight:700;">PROJ NO PATH</label><input type="text" id="mapProjNo" value="${state.scoroMapping.proj_no || 'entity.no'}" class="form-control"></div>
                    <div><label style="font-size:0.7rem; font-weight:700;">PROJ NAME PATH</label><input type="text" id="mapName" value="${state.scoroMapping.name || 'entity.project_name'}" class="form-control"></div>
                </div>
        ...
        <div class="glass-container" style="margin-bottom:24px;">
            ...
            <div id="userForm" class="settings-form"><input type="hidden" id="userId">
                <div style="display:grid; grid-template-columns:1fr 1fr; gap:16px; margin-bottom:16px;">
                    <div><label style="font-size:0.7rem; font-weight:700;">EMP NO</label><input type="text" id="userEmpNo" class="form-control"></div>
                    <div><label style="font-size:0.7rem; font-weight:700;">FULL NAME</label><input type="text" id="userName" class="form-control"></div>
                </div>
                <div style="display:grid; grid-template-columns:1fr 1fr; gap:16px; margin-bottom:16px;">
                    <div><label style="font-size:0.7rem; font-weight:700;">PASSWORD</label><input type="password" id="userPassword" class="form-control"></div>
                    <div><label style="font-size:0.7rem; font-weight:700;">DESIGNATION</label><input type="text" id="userDesignation" class="form-control"></div>
                </div>
                <div style="display:grid; grid-template-columns:1fr 1fr; gap:16px; margin-bottom:16px;">
                    <div><label style="font-size:0.7rem; font-weight:700;">DEPT</label><input type="text" id="userDepartment" class="form-control"></div>
                    <div><label style="font-size:0.7rem; font-weight:700;">SUB DEPT</label><input type="text" id="userSubDepartment" class="form-control"></div>
                </div>
                <div style="display:grid; grid-template-columns: 1fr 1fr; gap:16px; margin-bottom:16px;">
                    <div><label style="font-size:0.7rem; font-weight:700;">REPORTS TO</label><input type="text" id="userReportsTo" class="form-control"></div>
                    <div><label style="font-size:0.7rem; font-weight:700;">ACCESS ROLE</label><select id="userAccessRole" class="form-control"><option value="Employee">Employee</option><option value="Viewer">Viewer</option><option value="Editor">Editor</option><option value="Administrator">Administrator</option></select></div>
                </div>
        ...
        <div class="glass-container">
            ...
            <div id="projectForm" class="settings-form"><input type="hidden" id="projectId">
                <div style="display:grid; grid-template-columns:1fr 1fr; gap:16px; margin-bottom:16px;">
                    <div><label style="font-size:0.7rem; font-weight:700;">PROJECT NO</label><input type="text" id="projectNo" class="form-control"></div>
                    <div><label style="font-size:0.7rem; font-weight:700;">PROJECT NAME</label><input type="text" id="projectName" class="form-control"></div>
                </div>
                <div style="display:grid; grid-template-columns: 1fr 1fr; gap:16px; margin-bottom:16px;">
                    <div><label style="font-size:0.7rem; font-weight:700;">CLIENT</label><input type="text" id="projectClient" class="form-control"></div>
                    <div><label style="font-size:0.7rem; font-weight:700;">VESSEL</label><input type="text" id="projectVessel" class="form-control"></div>
                </div>
                <div style="display:grid; grid-template-columns: 1fr 1fr; gap:16px; margin-bottom:16px;">
                    <div><label style="font-size:0.7rem; font-weight:700;">BUDGET HRS</label><input type="number" id="projectBudget" class="form-control"></div>
                </div>
...
window.handleProjectSubmit = async () => {
    ...
    const data = { proj_no: projNo, name, client: document.getElementById('projectClient').value, vessel_name: document.getElementById('projectVessel').value, budget_hours: document.getElementById('projectBudget').value, id: 'proj_'+projNo };
...
window.deleteEmployee = async () => { const id = document.getElementById('userId').value; if (confirm('Delete?')) { await apiRequest(`/api/employees/${id}`, { method: 'DELETE' }); await initializeState(); renderSettings(document.getElementById('mainContent')); } };
window.deleteProject = async () => { const id = document.getElementById('projectId').value; if (confirm('Delete?')) { await apiRequest(`/api/projects/${id}`, { method: 'DELETE' }); await initializeState(); renderSettings(document.getElementById('mainContent')); } };
window.handleTimerProjectChange = (pid) => { const notes = document.getElementById('nptNotesContainer'); const project = state.projects.find(p => p.id === pid); if (project?.proj_no === '1000') notes?.classList.remove('hidden'); else notes?.classList.add('hidden'); };
window.filterTimerProjects = (query) => {
    const select = document.getElementById('timerProjectSelect');
    const q = query.toLowerCase();
    const filtered = state.projects.filter(p => (String(p.proj_no).toLowerCase().includes(q)) || (p.name?.toLowerCase().includes(q)));
    select.innerHTML = '<option value="">-- Select Project --</option>' + filtered.map(p => `<option value="${p.id}">[${p.proj_no}] ${p.name}</option>`).join('');
};
...
function setupGlobalEventListeners() {
    ...
    document.getElementById('loginForm')?.addEventListener('submit', (e) => {
        ...
        const emp = state.employees.find(e => String(e.emp_no) === empNo && e.password === pass);
