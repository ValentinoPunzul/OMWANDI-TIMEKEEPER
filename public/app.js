...
function renderViewHeader(title) {
    ...
    if (state.userRole === 'Employee' || state.userRole === 'Viewer' || isMobile()) {
        return `<div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:20px; width:100%;"><div><h1 style="margin:0; font-size:1.2rem; font-family:'Montserrat',sans-serif; font-weight:800;">OMWANDI <span style="color:var(--accent-primary)">Timekeeper</span></h1><p style="margin:0; font-size:0.75rem; color:var(--text-muted);">${me?.name || ''} (${state.userRole})</p></div>${logoutBtn}</div>`;
    }
...
function renderDashboard(container) {
    ...
    let html = filteredTimers.length === 0 ? '<div style="text-align:center; padding:40px; color:var(--text-muted);">No active sessions found.</div>' : 
        Object.entries(grouped).map(([pid, timers]) => {
            ...
            const list = timers.map(t => {
                ...
                return `<div class="timer-card glass-panel"><div class="timer-avatar" style="background:${emp.color}">${emp.avatar}</div><div class="timer-user-info"><div>${emp.name}</div><div style="font-size:0.8rem; opacity:0.7;">${emp.designation || ''}</div></div><div class="timer-counter" style="margin-right:15px; font-family:monospace;">${h}:${m}:${s}</div><button class="btn-text" style="color:#ef4444; font-weight:800;" onclick="stopUserTimer('${t.id}')">STOP</button></div>`;
            }).join('');
            return `<div class="project-group"><h3>[${proj.proj_no || '---'}] ${proj.name}</h3>${list}</div>`;
        }).join('');
    container.innerHTML = `${renderViewHeader('Dashboard')}<div class="clock-card glass-container"><div class="dashboard-time" id="dashboardTime">00:00:00</div><div class="dashboard-date" id="dashboardDate">LOADING...</div></div><div class="active-timers-section">${html}</div>`;
}
...
async function stopUserTimer(id) {
    ...
    if (!entry) return;
    const rawHours = Math.abs(new Date() - new Date(entry.start_time)) / 36e5;
    await apiRequest(`/api/entries/${id}`, { method: 'PUT', body: JSON.stringify({ end_time: new Date().toISOString(), total_hours: roundToQuarter(rawHours) }) });
...
function renderTimer(container) {
    ...
    const staffOptions = isForeman ? `<div style="text-align:left; margin-bottom:20px;"><label style="font-size:0.7rem; opacity:0.7; font-weight:700;">SELECT TEAM MEMBER</label><select id="timerStaffSelect" class="form-control"><option value="${me.id}">MYSELF (${me.name})</option>${myTeam.map(t => `<option value="${t.id}">${t.name}</option>`).join('')}</select></div>` : `<input type="hidden" id="timerStaffSelect" value="${state.activeProfileId}">`;
        ...
        <div class="timer-view-wrapper">
            ...
            <div class="timer-view-container glass-container">
                ...
                <div style="margin-bottom:20px; display:flex; align-items:center; justify-content:center; gap:8px;">
                    <span class="pulse-emerald"></span> <span style="color:#10b981; font-size:0.8rem; letter-spacing:1px; font-weight:700;">READY TO TRACK</span>
                </div>
                ${staffOptions}
                <div style="text-align:left; margin-bottom:20px;">
                    ...
                    <select id="timerProjectSelect" class="form-control" onchange="window.handleTimerProjectChange(this.value)"><option value="">-- Select Project --</option>${projects}</select>
                </div>
    ...
    container.innerHTML = `
        ...
        <div class="glass-container"><h3>Project Management</h3>
            ...
            <div id="projectForm" class="settings-form"><input type="hidden" id="projectId">
                ...
                <div style="display:grid; grid-template-columns:1fr 1fr; gap:16px; margin-bottom:16px;"><div><label style="font-size:0.7rem; font-weight:700;">BUDGET HOURS</label><input type="number" id="projectBudget" class="form-control"></div></div>
