/* ==========================================================================
   OMWANDI TIMEKEEPER - COMPLETE ENTERPRISE MASTER CLIENT
   ========================================================================== */
    ...
    if (state.userRole === 'Employee' || state.userRole === 'Viewer' || isMobile()) {
        return `
            <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:30px; width:100%;">
                <div>
                    <h1 style="margin:0; font-size:1.3rem; font-weight:800; font-family:'Montserrat', sans-serif;">OMWANDI <span style="color:var(--accent-primary)">Timekeeper</span></h1>
                    <p style="margin:2px 0 0 0; font-size:0.8rem; color:var(--text-muted);">${me?.name || ''} (${state.userRole})</p>
                </div>
                ${logoutBtn}
            </div>
        `;
...
function renderDashboard(container) {
    ...
    let html = activeTimers.length === 0 ? '<div style="text-align:center; padding:40px; color:var(--text-muted);">No active sessions found.</div>' : 
        Object.entries(grouped).map(([pid, timers]) => {
            ...
            const list = timers.map(t => {
                ...
                const startD = new Date(t.start_time);
                const diff = Math.floor((new Date() - startD) / 1000);
    ...
    container.innerHTML = `
        ...
        <div class="active-timers-section">${html}</div>
    `;
}
...
window.handleTimerProjectChange = (pid) => {
    ...
    if (project && String(project.proj_no) === '1000') notesContainer.classList.remove('hidden');
...
window.filterTimerProjects = (query) => {
    ...
    const filtered = state.projects.filter(p => (String(p.proj_no).toLowerCase().includes(q)) || (p.name && p.name.toLowerCase().includes(q)));
...
async function startTimer() {
    ...
    const project = state.projects.find(p => p.id === pid);
    let desc = 'Track Log';
    if (project && String(project.proj_no) === '1000') {
...
function renderProjects(container) {
    ...
    container.innerHTML = `${renderViewHeader('Projects')}<div class="projects-grid-v2">${html}</div>`;
}
...
function renderTimesheets(container) {
    const sortedEntries = [...state.timeEntries].sort((a, b) => {
        ...
        return state.timesheetSortDir === 'asc' ? String(valA).localeCompare(String(valB)) : String(valB).localeCompare(String(valA));
    });
    ...
    container.innerHTML = `${renderViewHeader('Timesheets')}<div class="glass-container"><div class="table-container"><table><thead><tr style="cursor:pointer;"><th onclick="setTimesheetSort('start_time')">Date ${getIcon('start_time')}</th><th onclick="setTimesheetSort('employee_name')">Member ${getIcon('employee_name')}</th><th onclick="setTimesheetSort('project_name')">Project ${getIcon('project_name')}</th><th>Start</th><th>End</th><th onclick="setTimesheetSort('total_hours')">Hours ${getIcon('total_hours')}</th></tr></thead><tbody>${rows}</tbody></table></div></div>`;
}
...
function handleProjectSelect(id) { const p = state.projects.find(p => p.id === id); if (!p) { resetProjectForm(); return; } document.getElementById('projectId').value = p.id; document.getElementById('projectNo').value = p.proj_no || ''; document.getElementById('projectName').value = p.name || ''; document.getElementById('projectClient').value = p.client || ''; document.getElementById('projectVessel').value = p.vessel_name || ''; document.getElementById('projectBudget').value = p.budget_hours || ''; document.getElementById('projectColor').value = p.color || '#6366f1'; const delBtn = document.getElementById('deleteProjectBtn'); if (delBtn) delBtn.style.display = 'inline-block'; }
