...
let _projViewMode = 'grid'; // 'grid' or 'list'
...
export async function renderProjects() {
    ...
    main.innerHTML = `
        ...
        ${_projViewMode === 'grid'
            ...
            : buildProjectListView(projects, isAdmin, customFields)
        }
...
function buildProjectListView(projects, isAdmin, customFields) {
    if (!projects.length) return '<p class="empty-state">No projects match your filters.</p>';

    const cols = ['proj_no','name','client','vessel_name','project_foreman','project_status','hours','budget'];
    const labels = ['No.','Project Name','Client','Vessel','Foreman','Status','Logged','Budget'];

    const headers = cols.map((col, i) => {
        const sortable = col !== 'vessel_name' && col !== 'project_foreman' && col !== 'project_status';
        const active = _projSortCol === col;
        const arrow = active ? (_projSortDir === 'asc' ? ' ↑' : ' ↓') : '';
        return `<th class="${active?'sort-active':''}" ${sortable?`style="cursor:pointer;user-select:none" onclick="sortProjBy('${col}')"`:''}>
            ${labels[i]}${arrow}
        </th>`;
    }).join('') + '<th></th>';

    const rows = projects.map(proj => {
        const burned = state.timeEntries.filter(e => e.project_id === proj.id && e.total_hours > 0).reduce((s,e) => s+e.total_hours, 0);
        const budget = proj.budget_hours || 0;
        const pct = budget > 0 ? Math.min((burned/budget)*100,100) : 0;
        const budgetClass = pct >= 100 ? 'danger' : pct >= 80 ? 'warning' : 'ok';
        const statusKey = (proj.status || proj.project_status || '').toLowerCase();
        const statusClass = {
            'inprogress':'status-inprogress','active':'status-inprogress',
            'done':'status-done','completed':'status-done',
            'notstarted':'status-notstarted','new':'status-notstarted',
            'cancelled':'status-cancelled','canceled':'status-cancelled',
            'onhold':'status-onhold','pending':'status-onhold',
        }[statusKey] || 'status-default';

        return `<tr>
            <td style="white-space:nowrap;font-weight:600">${escapeHtml(proj.proj_no||'')}</td>
            <td style="min-width:180px">${escapeHtml(proj.name||'')}</td>
            <td>${escapeHtml(proj.client||'')}</td>
            <td>${escapeHtml(proj.vessel_name||'—')}</td>
            <td>${escapeHtml(proj.project_foreman||'—')}</td>
            ...
            <td>
                ${isAdmin ? `
                <button class="btn-icon" onclick="editProject('${escapeHtml(proj.id)}')">✏️</button>
                <button class="btn-icon danger" onclick="deleteProject('${escapeHtml(proj.id)}')">🗑️</button>
                ` : ''}
            </td>
        </tr>`;
    }).join('');

    return `<div class="table-wrapper glass-panel">
...
window.sortProjBy = function(col) {
    ...
    else { _projSortCol = col; _projSortDir = 'asc'; }
    renderProjects();
};
